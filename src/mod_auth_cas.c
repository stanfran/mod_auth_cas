...
void setCASCookie(request_rec *r, char *cookieName, char *cookieValue, apr_byte_t secure, apr_time_t expireTime)
{
	...
	setCASCookieDetails(r, cookieName, cookieValue, secure, expireTime, c->CASCookieDomain, c);
}
...
void setCASGatewayCookie(request_rec *r, char *cookieName, char *cookieValue, apr_byte_t secure, apr_time_t expireTime)
{
	cas_cfg *c = ap_get_module_config(r->server->module_config, &auth_cas_module);
	if(c->CASDebug)
		ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r, "entering setCASGatewayCookie()");

	setCASCookieDetails(r, cookieName, cookieValue, secure, expireTime, c->CASGatewayCookieDomain, c);
}
...
void setCASCookieDetails(request_rec *r, char *cookieName, char *cookieValue, apr_byte_t secure, apr_time_t expireTime, char *cookieDomain, cas_cfg *c)
{
	...
	if(c->CASDebug)
		ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r, "entering setCASCookieDetails()");
...
apr_byte_t isValidCASCookie(request_rec *r, cas_cfg *c, char *cookie, char **user, cas_saml_attr **attrs)
{
	...
	*attrs = cas_saml_attr_pdup(r->pool, cache.attrs);
	...
	if(c->CASDebug)
		ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r, "idle time: %d", apr_time_msec(apr_time_now() - cache.lastactive));
	...
	if(ticket != NULL) {
		if(isValidCASTicket(r, c, ticket, &remoteUser, &attrs)) {
			...
			if (getCASCookie(r, d->CASGatewayCookie)) {
				setCASGatewayCookie(r, d->CASGatewayCookie, "TRUE", ssl, CAS_SESSION_EXPIRE_COOKIE_NOW);
			}
			r->user = remoteUser;
